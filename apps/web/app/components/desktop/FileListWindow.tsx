        ...
        entry.kind === "folder" ? (
          <ContextMenu key={`folder:${entry.name}`}>
            ...
            <ContextMenuContent className="s7-context-menu-content">
              <ContextMenuItem
                onSelect={() => setCurrentPath(currentPath ? `${currentPath}/${entry.name}` : entry.name)}
              >
                Open Folder
              </ContextMenuItem>
            </ContextMenuContent>
          ...
          <ContextMenu key={entry.file.id}>
            ...
            <ContextMenuContent className="s7-context-menu-content">
              <ContextMenuItem onSelect={() => onFileClick?.(entry.file.id)}>Open</ContextMenuItem>
              <ContextMenuItem onSelect={() => onGetInfo?.(entry.file.id)}>Get Info</ContextMenuItem>
              <ContextMenuSeparator />
              <ContextMenuItem variant="destructive" onSelect={() => onDeleteFile?.(entry.file.id)}>
                Delete
              </ContextMenuItem>
            </ContextMenuContent>
          </ContextMenu>
